...
def questions(qnum):
    if('user' in session):
        ...
        if(category == "intro"):
            ...
            bbogi=[3,4,5,7,10,12,14,15,19,24,30,32,36,39,40,41,43,45,50,62,65,70,80,100,120,150, 180, 210, 300, 320,  'ab','bd','ac','ad','bc','bcad','cadb','dbca','dacb','dabc']
...
def addAnswertoDB(qnum, answer):
    if('user' in session):
        ...
        category = get_CAT(int(qnum))
        c, conn = connectDB()
        query = "UPDATE " + category + " SET Q" + qnum + " = " + str(answer) + " WHERE userid = '" + userid + "';"
        c.execute(query)
        c.commit()
        conn.close()
